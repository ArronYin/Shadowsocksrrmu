...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget https://raw.githubusercontent.com/ArronYin/Shadowsocksrrmu/master/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget https://raw.githubusercontent.com/ArronYin/Shadowsocksrrmu/master/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Check_Libsodium_ver(){
	...
	Libsodiumr_ver=$(wget --no-check-certificate "https://github.com/jedisct1/libsodium/tags"|grep "/jedisct1/libsodium/releases/tag/"|head -1|sed -r 's/.*tag\/(.+)\">.*/\1/')
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		wget --no-check-certificate "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}/libsodium-${Libsodiumr_ver}.tar.gz"
		echo -e "${Info} 解压..."
